...
layer slices when `VLLM_ASCEND_PDMIX_LAYER_SLICE_SIZE` is set.
...
class LayerSliceInfo:
    ...
    When VLLM_ASCEND_PDMIX_LAYER_SLICE_SIZE > 0, the PassiveScheduler splits the local
...
class PassiveScheduler:
    ...
    def __init__(
        ...
        self._layer_slice_size = envs_ascend.VLLM_ASCEND_PDMIX_LAYER_SLICE_SIZE
        self._total_slices = 0
        self._num_local_layers = 0
        if self._layer_slice_size:
